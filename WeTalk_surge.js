//2026/04/21
// Surge 转换版 — 原版来自 ZenmoFeiShi/Qx
// 使用方式：
// [Script]
// WeTalk获取签到参数 = type=http-request, pattern=^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus, script-path=WeTalk_surge.js, timeout=60
// WeTalk签到 = type=cron, cronexp=20 8,20 * * *, script-path=WeTalk_surge.js, timeout=300
// [MITM]
// hostname = api.wetalkapp.com
/*
@Name：WeTalk 自动化签到+视频奖励 (Surge)
@Author：TG@ZenMoFiShi / Surge port by 铁子

[rewrite_local]
^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus url script-request-header WeTalk_surge.js

[task_local]
20 8,20 * * * WeTalk_surge.js, tag=WeTalk签到, enabled=true
...
function surgeFetch(options) {
  return new Promise((resolve, reject) => {
    ...
    const callback = (err, resp, data) => {
      if (err) {
        ...
        resolve({
          ...
          headers: resp && resp.headers ? resp.headers : {}
        });
      }
    };
    ...
    if (method === 'get') {
      ...
      $httpClient.put(req, callback);
    } else if (method === 'delete') {
      $httpClient.delete(req, callback);
    } else {
      $httpClient.get(req, callback);
    }
  });
...
function surgeNotify(title, subtitle, body) {
  $notification.post(title, subtitle || '', body || '');
}

function surgeStoreRead(key) {
  return $persistentStore.read(key);
}

function surgeStoreWrite(key, value) {
...
function loadStore() {
  const raw = surgeStoreRead(storeKey);
...
function saveStore(store) {
  surgeStoreWrite(storeKey, JSON.stringify(store));
}
...
function buildUA(baseUA, seed) {
  ...
  return `WeTalk/30.6.0 (com.innovationworks.wetalk; build:28; iOS ${pickItem(IOS_VERSIONS, seed)}) Alamofire/5.4.3`;
}
...
function notify(title, body) {
  surgeNotify(scriptName, title, body);
}
...
function runAccount(acc, index, total) {
  ...
  function fetchApi(path) {
    return surgeFetch({ url: buildUrl(path, acc.capture), method: 'GET', headers });
  }
